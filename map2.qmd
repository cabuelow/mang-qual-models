...
land_forecast <- st_read('data/typologies/Mangrove_Typology_v3_Composite_valid_centroids.gpkg') %>% 
  left_join(filter(dat2, cast == 'forecast' & var == 'LandwardMang'))
land_hindcast <- st_read('data/typologies/Mangrove_Typology_v3_Composite_valid_centroids.gpkg') %>% 
  left_join(filter(dat2, cast == 'hindcast' & var == 'LandwardMang'))
sea_forecast <- st_read('data/typologies/Mangrove_Typology_v3_Composite_valid_centroids.gpkg') %>% 
  left_join(filter(dat2, cast == 'forecast' & var == 'SeawardMang'))
sea_hindcast <- st_read('data/typologies/Mangrove_Typology_v3_Composite_valid_centroids.gpkg') %>% 
  left_join(filter(dat2, cast == 'hindcast' & var == 'SeawardMang'))
...
Due to memory limitations, only [subsidence](#subsidence), [sediment supply](#sediment-supply-and-future-dams), [future dams](#sediment-supply-and-future-dams), [tidal range](#tidal-range), [propagule establishment capacity](#tidal-range), [historical landward and seaward mangrove extent change](#historical-mangrove-loss-and-gain), and [landward](#landhind-forecast) abd [seaward](#seahind-forecast) are found on this page.
...
Hindcasts are mapped in the left pane, forecasts on the right

```{r echo=FALSE, fig.height=1, fig.width=1, message=FALSE, warning=FALSE, results=TRUE}

mapview(land_hindcast, zcol = 'Prob_change', layer.name = 'Landward extent change', cex = 0.9, lwd = 0) | mapview(land_forecast, zcol = 'Prob_change', layer.name = 'Landward extent change', cex = 0.9, lwd = 0)
...
mapview(sea_hindcast, zcol = 'Prob_change', layer.name = 'Seaward extent change', cex = 0.9, lwd = 0) | mapview(sea_forecast, zcol = 'Prob_change', layer.name = 'Seaward extent change', cex = 0.9, lwd = 0)
```