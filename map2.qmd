...
dat <- read.csv('outputs/master-dat.csv') %>% filter(sensitivity == 2) %>% select(Type:prop_estab)
...
land <- read.csv('outputs/validation/land-validation-results.csv') %>% 
  filter(ambig_threshold == 60) %>% 
  mutate(Land_hindcast = Land_Change)
sea <- read.csv('outputs/validation/sea-validation-results.csv') %>% 
    filter(ambig_threshold == 60) %>% 
    mutate(Sea_hindcast = Sea_Change)
...
landfor <- dat2 %>% 
  ...
  mutate(Land_forecast = ifelse(LandwardMang > threshold, 'Gain', NA),
         Land_forecast = ifelse(LandwardMang <= threshold & LandwardMang >= -threshold, 'Ambiguous', Land_forecast),
...
seafor <- dat2 %>% 
  ...
  mutate(Sea_forecast = ifelse(SeawardMang > threshold, 'Gain', NA),
         Sea_forecast = ifelse(SeawardMang <= threshold & SeawardMang >= -threshold, 'Ambiguous', Sea_forecast),
...
mapview(filter(typ_hind, !is.na(sea_change_obs)), zcol = 'sea_change_obs', layer.name = 'Seaward extent change', cex = 0.9, lwd = 0, map.type = 'Esri.WorldGrayCanvas') | mapview(filter(typ_hind, !is.na(land_change_obs)), zcol = 'land_change_obs', layer.name = 'Landward extent change', cex = 0.9, lwd = 0, map.type = 'Esri.WorldGrayCanvas')
...
mapview(filter(typ_hind, !is.na(Land_hindcast)), zcol = 'Land_hindcast', layer.name = 'Landward hindcast', cex = 0.9, lwd = 0, map.type = 'Esri.WorldGrayCanvas') | mapview(filter(typ_fore, !is.na(Land_forecast)), zcol = 'Land_forecast', layer.name = 'Landward forecast', cex = 0.9, lwd = 0, map.type = 'Esri.WorldGrayCanvas')

```

Hindcasts of **landward mangrove losses** have an overall accuracy of `r round(calc_accuracy(land_validate$Land_Loss, land_validate$land_loss_obs)$accuracy.results[1,2])`% (commission accuracy = `r round(calc_accuracy(land_validate$Land_Loss, land_validate$land_loss_obs)$accuracy.results[1,4])`% and omission accuracy = `r round(calc_accuracy(land_validate$Land_Loss, land_validate$land_loss_obs)$accuracy.results[1,3])`%).

Hindcasts of **landward mangrove gains** have an overall accuracy of `r round(calc_accuracy(land_validate$Land_Gain, land_validate$land_gain_obs)$accuracy.results[1,2])`% (commission accuracy = `r round(calc_accuracy(land_validate$Land_Gain, land_validate$land_gain_obs)$accuracy.results[1,4])`% and omission accuracy = `r round(calc_accuracy(land_validate$Land_Gain, land_validate$land_gain_obs)$accuracy.results[1,3])`%).

*Note* High commission accuracy means that where gains or losses are predicted to occur, they are correct most of the time. Alternatively, high omission accuracy means that most of the time the model predicts correctly where gains or losses won't occur.
:::
...
mapview(filter(typ_hind, !is.na(Land_hindcast)), zcol = 'Land_hindcast', layer.name = 'Seaward hindcast', cex = 0.9, lwd = 0, map.type = 'Esri.WorldGrayCanvas') | mapview(filter(typ_fore, !is.na(Sea_forecast)), zcol = 'Sea_forecast', layer.name = 'Seaward forecast', cex = 0.9, lwd = 0, map.type = 'Esri.WorldGrayCanvas')
```

Hindcasts of **seaward mangrove losses** have an overall accuracy of `r round(calc_accuracy(sea_validate$Sea_Loss, sea_validate$sea_loss_obs)$accuracy.results[1,2])`% (commission accuracy = `r round(calc_accuracy(sea_validate$Sea_Loss, sea_validate$sea_loss_obs)$accuracy.results[1,4])`% and omission accuracy = `r round(calc_accuracy(sea_validate$Sea_Loss, sea_validate$sea_loss_obs)$accuracy.results[1,3])`%).

Hindcasts of **seaward mangrove gains** have an overall accuracy of `r round(calc_accuracy(sea_validate$Sea_Gain, sea_validate$sea_gain_obs)$accuracy.results[1,2])`% (commission accuracy = `r round(calc_accuracy(sea_validate$Sea_Gain, sea_validate$sea_gain_obs)$accuracy.results[1,4])`% and omission accuracy = `r round(calc_accuracy(sea_validate$Sea_Gain, sea_validate$sea_gain_obs)$accuracy.results[1,3])`%).
