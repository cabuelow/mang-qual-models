...
typo <- vect("data/typologies/v3.14/Mangrove_Typology_v3.14_Composite_valid_EEZ_qgis.gpkg")


## Method 1: using GMW 2020 raster layer mask over typologies polygons
results <- data.frame(Type_MRGID = NA, clumpy = NA)

# loop process for caculating clumpiness
# exclude fiji's 60299 from the loop process
typo_sub <- typo[!typo$Type == "OpenCoast_60299",]
for (i in 1:nrow(typo_sub)) {
  gmw_mask <- crop(gmw, typo_sub[i,], mask = TRUE) # crop and mask by typology unit
  ...
  results[i,1] <- values(typo_sub[i,"Type_MRGID"]) 
...
# # For Fiji 60299
# Fiji type == "OpenCoast_60299" 
fiji <- typo[typo$Type == "OpenCoast_60299",]
...
results <- rbind(results, result_fiji)
write.csv(results, 'outputs/processed-data/clumpiness.csv', row.names = F)
