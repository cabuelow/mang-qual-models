...
typo <- st_read("data/typologies/Mangrove_Typology_v3_Composite_valid.gpkg") %>%  st_buffer(0)

value <- exact_extract(dist, typo, c('mean', 'min', 'max', 'stdev'))
results <- data.frame(Type = typo$Type, value)

## replace na with max distance in the entire dist raster
results1 <- results %>% 
...
## replace na with max distance of the nearest polygon
...
nearest <- data.frame(Type = typo_na$Type, 
                      Type_nearest = typo_non_na$Type[st_nearest_feature(typo_na, typo_non_na)])

typo_na_results <- left_join(nearest, results, by = join_by(`Type_nearest` == `Type`)) %>% 
  select(Type, max)
colnames(typo_na_results)[2] <- "max.y"

results2 <- left_join(results, typo_na_results, by = join_by(Type)) %>% 
  mutate(mean = ifelse(is.na(mean), max.y, mean))
results2 <- select(results2, -c(max.y))

write.csv(results2, 'outputs/processed-data/distance-to-city-na-nearest-max.csv', row.names = F)
