...
simA_all <- system.simulate(1000, modelA,
                            validators = list(
                              press.validate(modelA, perturb=c(LandwardLatAccom=-1,
                                                               SeawardLatAccom=1,
                                                               LandwardPropag=-1,
...
simAsimp_all <- system.simulate(1000, modelAsimp,
                            validators = list(
                              press.validate(modelAsimp, perturb=c(LandwardLatAccom=-1,
                                                               SeawardLatAccom=1,
                                                               #LandwardPropag=-1,
...
params$scnr <- rep(c("SLR", "Sediment supply", "Sediment supply & SLR", "Sediment supply & SLF", 'All'), each = 1000)
...
ggplot() +
  geom_density(data = filter(params, scnr == 'SLR'), 
               aes(x = weight), colour = 'blue', fill = 'blue', alpha = 0.5) +
   geom_density(data = filter(params, scnr == 'Sediment supply'), 
               aes(x = weight), colour = 'yellow', fill = 'yellow', alpha = 0.5) +
   geom_density(data = filter(params, scnr == 'Sediment supply & SLR'), 
               aes(x = weight), colour = 'green', fill = 'green', alpha = 0.5) +
   geom_density(data = filter(params, scnr == 'Sediment supply & SLF'), 
               aes(x = weight), colour = 'red', fill = 'red', alpha = 0.5) +
   geom_density(data = filter(params, scnr == 'All'), 
               aes(x = weight), colour = 'purple', fill = 'purple', alpha = 0.5) +
  facet_wrap(~link, scales = 'free')

```
So they are all pretty uncertain. Probably can't do anything with this?
