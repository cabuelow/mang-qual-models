...
dat <- spatial_dat %>% # get unique biophysical/pressure combinations historically, given 5 different pressure definitions
  ...
  dplyr::select(pressure_def, Type, csqueeze, csqueeze_1, sed_supp, Tidal_Class, prop_estab, #ant_slr, 
                gwsub, hist_drought, hist_ext_rain, storms, climate, cdev) %>% 
...
bio_dat <- dat %>% select(csqueeze:cdev, scenario) %>% distinct() # unique biophysical contexts
...
saveRDS(tmp, paste0('outputs/simulation-outcomes/scenario_matrices.RDS'))
...
naive_outcomes <- rbind(naive_outcomes, data.frame(nsim = rep(1:nsim, nrow(d)), 
                                                   ...
                                                   LandwardMang = 1,
                                                   SeawardMang = 1))
write.csv(naive_outcomes, paste0('outputs/validation/naive_outcomes.csv'), row.names = F)
...
stopCluster(cl)
saveRDS(results, paste0('outputs/validation/accuracy.RDS'))
...
ggsave(paste0('outputs/validation/accuracy-heatmap_kfold.png'), width = 15, height = 11)
